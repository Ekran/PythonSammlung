...
dirname = os.path.dirname(os.path.realpath(__file__)) # http://stackoverflow.com/questions/5137497/find-current-directory-and-files-directory
...
def get_Protokoll_dateien(dirname):
    ...
    os.access(dirname, os.R_OK) == False):
		...
		objects.sort()
		files = list() # Liste mit pro ZEile: Dateiname, versuchnummer, Matrikel, Gruppennummer
		for objectname in objects:
			...
			if (pos_appendix  > 0): 
				...
				files.append([objectname[0:pos_appendix],Versuch,Matrikel.upper(),Gruppe])
...
def get_Anzahl_Gruppen_in_Matrikel(Matrikel, files):
	...
	for file in files:
		if (file[2].upper() == Matrikel.upper()):
			if (anzahl_Gruppen < int(file[3])):	## bisherige gespeicherte Anzahl kleiner als Aktuelle Gruppennummer? 
...
for Matrikel in alle_Matrikel:
	...
	for Gruppe in range(1,Gruppenanzahl+1):
		...
		print("\tGruppe " + str(Gruppe) + ":\t" + str(v1) + "\t" + str(v2) + "\t" + str(v3) + "\t" + str(v4) + "\t")
